...
module.exports = {
  ...
  createNewEntry: async (req, res) => {
    try {
      ...
      console.log(rawData);
      const data = await expenseCategoriesModule.create(rawData);
  ...
  updateEntry: async (req, res) => {
    try {
      const find = await expenseCategoriesModule.findByPk(req.params.id);
      if (!find)
        return res.status(200).json({
          status: 200,
      ...
      const rawData = req.body;
      rawData.updated_at = new Date();
      const updateed = await expenseCategoriesModule.update(rawData, {
        where: {
          id: req.params.id,
        },
      });
